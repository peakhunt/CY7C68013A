...
extern void timer0_isr(void) __interrupt (TF0_ISR);
...
void
resume_isr(void) __interrupt (RESUME_ISR)
{
  CLEAR_RESUME();
}

void
sudav_isr(void) __interrupt (SUDAV_ISR)
{
  dosud=TRUE;
  CLEAR_SUDAV();
}
...
void
usbreset_isr(void) __interrupt (USBRESET_ISR)
{
...
void
hispeed_isr(void) __interrupt (HISPEED_ISR)
{
  handle_hispeed(TRUE);
  CLEAR_HISPEED();
}
